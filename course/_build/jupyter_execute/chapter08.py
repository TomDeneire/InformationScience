...
YottaDB also provide a C and Go wrapper, so you can access the MUMPS database without using MUMPS, if you want. You see, MUMPS is an old language and has a pretty __[bad reputation](https://thedailywtf.com/articles/A_Case_of_the_MUMPS)__, not completely undeservedly to be frank. On the other hand, MUMPS is very simple, quite fast and we love it.
...
The `p` in `brocade.phtml` stands for PHP, it is a HTML file with can also execute PHP code. PHP (unlike Javascript) runs *server side* which means it can access the server's shell. The shell can then start a MUMPS process that the input HTML (e.g. username and password), processes this HTML (e.g. lookup access rights in the database) and the produces output HTML over stdout. This is then read by PHP again to enable the server to render it on screen again.

### Python

Our server also has a Python installation, in fact: we still need both Python2 and Python3. In Brocade, Python is used for many different things, but one of its main purposes is to run what we call **toolcat applications**.
